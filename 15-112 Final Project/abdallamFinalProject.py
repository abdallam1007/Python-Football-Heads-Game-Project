...
class Particle():
	...
	def move(self):
		...
		# we here use the vector componnent analysis to find 
		# the x and y coordenates of the ball
		self.ballX += math.sin(self.angle) * self.speed
		self.ballY -= math.cos(self.angle) * self.speed
		# we take into consederation the speed lost while moving 
		# the ball through the air
		# the faster the ball the more speed lost
		self.speed *= drag
	...
	def goalBounce(self,goalLR,goalRR,goalW,goalH):
		...
		if goalLR.colliderect(ballRect):
			if self.ballY > goalH:
				# here we calculate how far did the ball go outside the boundries 
				# then we reflect the cordanetes to make the ball look like it bounced
				self.ballX = 2*self.ballSize - self.ballX
				# the vertical boundary has an angle of 0, therefore we reflect the angle 
				# by subtracting the current angle from 0 (reverse it)
				self.angle = - self.angle
				# we consider the loss of speed when the ball hits 
				# the boundries of the playground
				self.speed *= elasticity
			if self.ballY <= goalH:
				# here we calculate how far did the ball go outside the boundries 
				# then we reflect the cordanetes to make the ball look like it bounced
				self.ballY = 2*(goalH - self.ballSize) - self.ballY
				# the horizontal boundray have an angle of pi, thatswhy 
				# we subtract the current angle from pi
	...
	def collide(self,leadX1,leadY1,leadXChange1):
		# when the ball and the player collide they touch at a small point 
		# the angle is the tangent at that point 
		# we calculate the tangent to calculate the angle 
		...
		# here we exchange the speed of both the player and the ball
		# so they move after the collesion
		...
		# we change the x and y coordenates accordingly 
